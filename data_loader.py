...
class MVTecDRAEM_Test_Visual_Dataset(Dataset):
    ...
    def __init__(self, root_dir, resize_shape=None):
        self.root_dir = root_dir
        # self.images = sorted(glob.glob(root_dir+"/*/*.png"))[:2]//限制只取2張圖
        self.images = sorted(glob.glob(root_dir + "/*/*.png"))
        self.resize_shape = resize_shape
...
class VisADRAEM_Test_Visual_Dataset(Dataset):
    """VisA Dataset 的測試視覺化資料集，結構與 MVTec 類似但支援 JPG/PNG 混合格式"""

    def __init__(self, root_dir, resize_shape=None):
        self.root_dir = root_dir
        # VisA 圖片可能是 .JPG, .jpg, 或 .png
        self.images = sorted(
    ...
    def __getitem__(self, idx):
        ...
        if base_dir == 'good':
            ...
            mask_path = os.path.join(dir_path, '../../ground_truth/')
            mask_path = os.path.join(mask_path, base_dir)
            # VisA mask 檔名: 去掉原始副檔名後加 _mask.png
            file_stem = os.path.splitext(file_name)[0]
            mask_file_name = file_stem + "_mask.png"
            mask_path = os.path.join(mask_path, mask_file_name)
            if not os.path.exists(mask_path):
                # 備用: 直接用同檔名 .png
                mask_path = os.path.join(
                    dir_path, '../../ground_truth/', base_dir, file_stem + ".png")
            image, mask = self.transform_image(img_path, mask_path)
            has_anomaly = np.array([1], dtype=np.float32)
